...
public sealed class CallCenterChannelHandler : IChannelHandler
{
    public ChannelType SupportedChannelType => ChannelType.CallCenter;
    ...
    public Task<SendResult> SendReplyAsync(ChannelMessage message, ChannelDefinition definition, CancellationToken ct = default)
        => Task.FromResult(SendResult.Ok($"callcenter-{Guid.NewGuid():N}"));

    public ChannelContext ExtractContext(object rawMessage, ChannelDefinition definition)
        => ChannelContext.Create(definition.Type, definition.Id, Guid.NewGuid().ToString("N"), "callcenter-user", "Call center user");
    ...
    public Task<HealthStatus> CheckHealthAsync(ChannelDefinition definition, CancellationToken ct = default)
    {
        ...
        return Task.FromResult(hasConnection
            ? HealthStatus.Ok($"Call center channel ready using {provider}.")
            : HealthStatus.Unhealthy("Call center channel requires a reusable Twilio connection."));
    }
}
